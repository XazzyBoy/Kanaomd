...
async function start(oldSocket = null, opts = { store, logger, authState }) {
	...
	if(useMobile && !conn.authState.creds.registered) {
		...
		if(!registration.phoneNumber) {
			registration.phoneNumber = await question(chalk.blueBright('Input a Valid number start with region code. Example : 62xxx:\n'))
		}
...
async function connectionUpdate(opts, update) {
  ...
  message: {
    conversation: "*[ Notifikasi bot aktif ]*"
  }
};

let tek = `Hai sayang, aku tersambung!\n\nScript Ini Di Tulis Ulang Oleh Im-Dims\nWhatsapp: wa.me/6281398274790`;
    ...
    if (update.receivedPendingNotifications) conn.sendMessage(`6281398274790@s.whatsapp.net`, { text: tek }, { quoted: q })  
}

function getMessageConfig() {
    	const welcome = 'Selamat Datang *@user* Di *@subject*\n\nMohon Di Baca Ya!\n@desc'
