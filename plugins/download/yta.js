...
if (!args[0].match(new RegExp(/(?:https?:\/\/)?(?:youtu\.be\/|(?:www\.|m\.)?youtube\.com\/(?:watch|v|embed|shorts)(?:\.php)?(?:\?.*v=|\/))([a-zA-Z0-9\_-]+)/, 'gi'))) return m.reply(`Url Youtube tidak valid.`)
try {
...
console.log(e)
try {
let data = await fetch(`https://api.azz.biz.id/api/play?q=${text}&key=global`)
let res = await data.json()
conn.sendMessage(m.chat, { document: { url: res.mp3 }, mimetype: 'audio/mpeg', fileName: res.videoInfo.title, caption: 'Sukses kak >////<' }, {quoted: m})
} catch {  	
console.log(e)
try {
conn.sendMessage(m.chat, {audio : {url : `https://aemt.me/youtube?url=${text}&filter=audioonly&quality=highestaudio&contenttype=audio/mpeg`}, mimetype:'audio/mpeg'}, {quoted:m})
